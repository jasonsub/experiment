...
On the other hand, the user of conventional model checker will have to write all these clauses.
These clauses contain cross-literals, e.g. $s_2$ may associate with $a_1, p_0$, etc. If the user 
is not familiar with the implementation of this adder, those cross-literals will bring inconveniences
and confusions. However, if word-level techniques allow specification like equation \ref{eqn:wordspec},
the verification tool will be very user-friendly and straightforward.
...
An old but universally effective abstraction method is {\bf Lagrange's interpolation}. Instead of using 
...
\end{equation}
The geometry meaning of Lagrange's interpolation in real algebra is: given $N$ dots with coordinates $(x_i,y_i)$,
they can always be fitted into a polynomial function with at most $N-1$ degree, and that function can be 
written in the form of equation \ref{eqn:Lagrange}. In this example, although defined in Galois field instead of 
real number field, the essential concept of Lagrange's interpolation keeps the same. We can get 8 ``dots":
\begin{align*}
Uni-form&: (a_2\alpha^2+a_1\alpha+a_0,z_2\alpha^2+z_1\alpha+z_0) \gets (A,Z) \\
Dot~1&: (0, 0) \gets (000,000) \\
Dot~2&: (1,1) \gets (001,001) \\
Dot~3&:  (\alpha,\alpha^2 + \alpha + 1) \gets (010,111)\\
Dot~4&:  (\alpha + 1,\alpha^2 + \alpha + 1)\gets (011,111) \\
Dot~5&:  (\alpha^2,\alpha^2+1) \gets (100,101)\\
Dot~6&:  (\alpha^2 + 1,\alpha+1)\gets (101,011)\\
Dot~7&:  (\alpha^2 + \alpha,\alpha^2 + 1)\gets (110,101)\\
Dot~8&:  (\alpha^2 + \alpha + 1,\alpha^2 + 1)\gets (111,101)
\end{align*}
Substitute 8 $(x_i,y_i)$ pairs in equation \ref{eqn:Lagrange} with these 8 ``dots" in $\F_{2^3}$.
...
In practice, Lagrange's interpolation is too weak on scalability. Our approach uses abstraction based on 
Gr\"obner basis with abstraction term order (ATO), which is briefly introduced in preliminaries.
...
\label{sec:improve}
% TODO Revised according to comments in next chapter

Using ATO on computing GB for large set of polynomial is time-consuming, and usually 
...
The authors of \cite{pruss:tcad} propose a method to construct 
a system of equations and solution to the system consists of equation \ref{eqn:BLVS_reacha}.
One way to obtain  is to apply a Gaussian-elimination-fashion approach, which could 
...
\end{bmatrix}
\end{align}
Treat ${\bf s}$ as a vector of $k$ unknowns $s_0,\dots,s_{k-1}$,
then equation \ref{eqn:betamat} can be solved by using Cramer's rule or Gaussian elimination.
In other words, we can obtain $s_i=\mathcal G(S)$ by solving equation \ref{eqn:betamat} symbolically.
...
$J_0^{PI} = \langle x_1^2-x_1,\dots,x_n^2-x_n\rangle$. Using elimination term order with $S>x_i>T$,
we can compute a GB of the elimination ideal $\langle f_T,f_S\rangle + J_0^{PI}$. This GB contains a univariate
polynomial denoting next states. The improved algorithm is depicted in Alg. \ref{alg:refined}.
...
\begin{Proposition}
Divide $n$-bit PIs to partitions $n_1,n_2,\dots, n_s$ and let $n_1$-bit, $n_2$-bit, $\dots$, $n_s$-bit word-level variables
represent their evaluations in $\mathbb F_{2^k}$ respectively. Then $n_1,n_2,\dots,n_s~|~k$.
\end{Proposition}
...
The following example illustrates the full-blown reachability analysis approach
based on C++ and \textsc{Singular} implementation.

\begin{Example}[Full Blown Reachability Analysis Walk-through]
