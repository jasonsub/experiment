...
\bi
\item Word-level reachability analysis -- analog if implicit state enumeration
	\bi 
	\item {\bf Q:} Why word-level? 
	\pause
	\item {\bf A:} Datapath $\gets$ word-level info 
	\pause
	\item {\bf A:} Simplify representation (abstraction) of state-space $\to$ Efficiency!
	\ei
\pause
\item Apply word-level reachability algo. to sequential arithmetic circuit verification
	\bi
...
\item UNSAT cores in Algebraic geometry
	...
	\item Apply UNSAT core to $k$-BMC abstraction refinement
	\ei
\pause
...
\pause
\bi
\item A polynomial function representation always exists in $\Fq[A]$ (Lagrange's interpolation)
\ei
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{Theorem}
\bi
\item Given ideal $J$, compute $J'$ s.t. $V(J') = \overline{V(J)}$
\item $\to J'=J_0:J$
\ei
\end{frame}
