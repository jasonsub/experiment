\chapter{Functional Verification of Sequential Normal Basis\protect\\ Multipliers}
\label{ch:normal}
With the size of sequential arithmetic circuits increasing rapidly, 
conventional methods become inefficient for verifying the correctness of these circuits.
...
which we can apply to our traversal algorithm to actually 
...
However, it also brings difficulties in verifying the circuit functions.
...
the specification function, as shown in Figure \ref{fig:convention}. However, the number of gates will grow fast when unrolling
at bit level. Meanwhile the structural similarity-based equivalence checking techniques 
...
the concept of ``RATO", which was previously used in Section \ref{sec:improve}.
...
explicit unrolling and avoid a bit-blasting problem.
...
For 2-input sequential NB multipliers, abstraction is utilized to implement Algorithm \ref{alg:modified}.
Notations in Figure \ref{fig:sequential} as well as in the next example comply with 
...
\begin{figure}[bp]
\centering{
\includegraphics[width=4.5in]{newfig/gf_seq_model.eps}
...
Then the implicit FSM unrolling problem is set up for abstraction. 
...
allows us to eliminate all the bit-level variables 
...
all gates in a combinational logic block and definition of word-level variables.
...
$$R$$
The other two polynomials describe PS variable $A,B$ -- current multiplication operands, and we write them 
in parameters $A_{init}, B_{init}$. Since this is the first iteration, they have the same form as $f_{16}$ and $f_{17}$:
\begin{align*}
...
in Table \ref{tab:slimgb}. It indicates that our approach based on ATO cannot verify sequential GF multiplier with size larger than 11 bits operands.
...
derived from the circuit structure, followed by word-level NS outputs (e.g., $R'$ in Figure \ref{fig:sequential}), 
...
\end{align}
The variables in braces are at the same topological level and are arranged in LEX order. 
...
% \end{align}
to recursively eliminate $a_1$ from the third row, $a_2$ from the fourth row, etc. The final solution to this system of
equations is
%\begin{displaymath}
...
only one term containing word-level output with degree equal to 1.
\end{Proposition}

\begin{Proof}
The first proposition is easy to prove by contradiction: Assume there exists an intermediate bit-level variable $v$ in the remainder,
...
will appear in remainder $r$, with exponent equal to 1.
\end{Proof}

By replacing all bit-level variables with corresponding word-level variable polynomials, we transform the remainder
of $Spoly$ reduction to the form of $R'+R+\mathcal F'(A,B)$. Note $R$ is the present state notion of output, which
equals to initial value $R=0$ in first clock cycle, or value of $R'$ from the last clock cycle. By substituting
...
\end{equation}
We use an example to help understand the notations in Equation \ref{eqn:Moore}:
\begin{Example}
...
\end{align}
First, let $i=1$,  we obtain $c_0\in \mathbb F_2$. When $c_0=0$, the product term equals $\alpha_1$; when $c_0=1$ it equals $(\alpha_1-\alpha_0)$.
...
of all elements of a field equals 1, the Moore determinant $\M=1$.
\end{Proof}
...
computation in the main loop is divided into 4 different functional parts:

1) Preprocessing:

This step is executed only once before the main loop starts. The function of preprocessing is to compute the following system
...
in the following steps). Our C++ tool does not have a data size limit, but it accumulates exponents when computing 
...
method (whose time complexity is $O(k^3)$), the time cost is trivial comparing to the following steps. 
...
For SMPO experiments, reduced $Spoly$ has the following generic form (all coefficients are omitted):
\begin{equation}
\label{eqn:redSpoly}
...
there exist no cross-product terms simultaneously containing elements within the same set, i.e., sets
...
after substituting a bit-level variable with a corresponding word-level variable.
...
Use the result from preprocessing, get rid of $r_i, a_i$ and $b_i$ through substitution. This step yields the following polynomial:
\begin{equation}
\label{eqn:HABR}
...
with only the leading term replaced in step ``$from^i\gets to^i$" in 
...
A (preverified) $\Fkk$ Mastrovito multiplier \cite{mastro:1989} is
...
\begin{table}[bp]
\centering
\caption{Run-time for verification of bug-free RH-SMPO circuits
...
\begin{table}[bp]
\centering
\caption{Similarity between RH-SMPO and Agnew's SMPO}
\label{tbl:fraig}
\begin{tabular}{|c||c|c|c|c|} 
\hline
Size $k$ & 11 & 18 & 23 & 33 \\
...
\begin{table}[tbp]
\centering
\caption{Run-time (seconds) for verification of bug-free and
...
\begin{table}[tbp]
\centering
\caption{Run-time (seconds) for verification of
...
multipliers over $\Fkk$ using an approach based on computer algebra and algebraic geometry. 
As sequential Galois field circuits perform the
...
word-level multivariate polynomial. Using algebraic geometry, we show
...
circuits, whereas contemporary techniques (e.g., BDDs) fail beyond 23-bit
datapaths.  
