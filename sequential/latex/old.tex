...
\title{Sequential Verification over Gal\"ois Field based on Word-level Abstraction}
...
Sequential Multiplier with Parallel Output (SMPO) designed for normal basis multiplication.
\end{abstract}
...
Let $\beta$ be one element in Gal\"ois Field $GF(2^n)$ constructed by primitive element $\alpha$ and minimal polynomial 
$f(\alpha)$, then a set of bases in the form $\{\beta, \beta^2, \beta^4, \beta^8, ... ,\beta^{2^{n-1}}\}$ can be named
as \emph{Normal Basis}; here $\beta$ is called \emph{Normal Element}.\\

More about normal element theory please refer to \textit{ref1: write-up about normal basis}.

	\subsection{Normal Basis Multiplication}
For arithmetics in Gal\"ois fields, multiplication (with modulus) can be greatly simplified if Normal Basis is adopted
to represent operands. Please look at several examples below:\\

\textit{Example 1:}\ \ Element square: In $GF(2^n)$, we have relation $(a+b)^2 = a^2 + b^2$. Then 
$(b_0\beta + b_1\beta^2 + b_2\beta^4 + \dots + b_{n-1}\beta^{2^{n-1}})^2$ = ? \\

\textit{Example 2:}\ \ Normal basis multiplication: Suppose we have 2 binary vector represent 2 operands in normal
...
\textit{Example 3:}\ \ Multiplication table: We can use a binary $n\times n$ matrix $M_T$ to describe the "function"
 mentioned above: $c_{n-1} = f(A, B) = A \cdot M_T \cdot B^T$, $B^T$ denotes vector transposition. The definition
of multiplication table is here:\\
...
\beta^{2^{n-1}} 
\end{array} \right)$. \\
The number of "1" in $M_T$ is known as  \emph{Complexity$(C_N)$}. Easy to prove that $C_N \geq 2n - 1$.\\
...
For 5-bit SMSO, consider following equation \\
$c_i = b_ia_{i+1} + b_{i+1}(a_i + a_{i+3}) + b_{i+2}(a_{i+3} + a_{i+4}) + b_{i+3}(a_{i+1} + a_{i+2}) + b_{i+4}
(a_{i+2} + a_{i+4}), 0\leq i\leq 4$\\
It is possible to calculate every product term within one clock. Also utilize cyclic similarity, we have following
design:
\begin{figure}
\centering
...
optimal design. If there exists a set of normal basis satisfying $C_N = 2n - 1$, this normal basis is named as
\emph{Optimal Normal Basis}.\\
There are 2 types of optimal normal basis (about optimal normal basis, refer to \textit{ref3:ONB}) \cite{ONB} \\
...
\hspace{8mm}\par
Here we have another problem: can we use these definitions to compute optimal normal element within $O(1)$?

\section{Methodology}
\begin{itemize}
\item \textbf{Step 1}\ \ Calculate Multiplication table, using equation ((i,j) is coordinate in matrix)\\
		$2^i\pm 2^j \equiv \pm1 mod (2n + 1)$; \cite{KAUST}
...
\item \textbf{Problem 2}\ \ What's the correspondence between matrix for multiplication and "officially-defined"
