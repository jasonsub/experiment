...
\SetAlgoNoLine
 ...
 $\F(R), \mathcal{G}(A_{init}), \mathcal{H}(B_{init})$}
  ...
  {
  	$i \gets i + 1$\;
	$to^i(R',A',B') \gets$GB w/ abstraction term order$\langle J_{ckt}, J_0, from^{i-1}(R,A,B)\rangle$\;
...
Line 6 in algorithm \ref{alg:modified} is implemented by replacing $R'$ with $R$, $\{a_i', b_j'\}$ with $\{a_i,b_j\}$.
